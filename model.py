...
from multi_class import get_data_loader 
...
def train_val(model, params):
    ...
    for epoch in range(num_epochs):
        current_lr = get_lr(opt)
        print("Epoch %d/%d, current lr = %d" % (epoch, num_epochs - 1, current_lr))
        ...
        model.eval()
        with torch.no_grad():
            val_loss, val_metric = loss_epoch(model, device, loss_func, val_dl, sanity, opt)

            loss_history["val"].append(train_loss)
            metric_history["val"].append(train_metric)
            ...
            lr_scheduler.step()
            print("train loss: %.6f, dev loss: %.6f, accuracy: %2.f" %
...
if __name__ == "__main__":
    ...
    train_dl, val_dl =get_data_loader("./data")
    ...
    params_train = {
            ...
            "path_to_weights" : "./models/resnet18.pt",
            "device" : device
            }
